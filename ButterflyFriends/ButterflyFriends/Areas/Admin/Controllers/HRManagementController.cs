    ...
    public class HRManagementController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var model = new HRManagamentModel
            {
                ...
                SponsorModel =
                    new SponsorModel {Sponsors = users.Where(s => s.Employee == null).ToPagedList(1, pageSize)}
            };
            return View(model);
        }
        ...
        public ActionResult ProfilePictureUpload()
        {
            ...
            if (Request.Files.Count > 0)
                try
                {
                    ...
                    {
                        child = _context.Children.Find(int.Parse(userId)); //get child id
                    ...
                    for (var i = 0; i < files.Count; i++)
                    {
                        ...
                        if (profPic.Content == null)
                        {
                            ...
                            var ratio = bmp.Width/(double) bmp.Height; //get height width ratio
                            var height = (int) (ProfileImageWidth/ratio);
                            var newImg = ResizeImage(bmp, ProfileImageWidth, height);
                            //resize image so that it cannot exceed 300 pixels
                            ...
                            //resize profile image to 40 pixels to create a thumbnail
                            byte[] Tcntnt;
                            using (var stream = new MemoryStream())
                            {
                        ...
                        else
                        {
                            ...
                            var ratio = bmp.Width/(double) bmp.Height; //ratio of width and height
                            var height = (int) (ProfileImageWidth/ratio);
        ...
        public ActionResult EmployeeList(int? employeePage)
        {
            ...
            var pageNumber = employeePage ?? 1;
            ViewBag.employeePage = employeePage ?? 1;

            return FilterResultEmployees(search, active, order, filter, employeePhone, EmployeeStreetadress, EmployeeZip,
                EmployeeCity, EmployeeCounty, EmployeePosition, employeeAccountNumber, pageNumber);
        }
        ...
        public ActionResult SponsorList(int? sponsorPage)
        {
            ...
            var pageNumber = sponsorPage ?? 1;
            ViewBag.sponsorPage = sponsorPage ?? 1;

            return FilterResultSponsors(search, active, order, filter, SponsorPhone, SponsorStreetadress, SponsorZip,
                SponsorCity, SponsorCounty, pageNumber);
        }
        ...
        public ActionResult showEmployeeDetails(string id, string type, int? employeePage)
        {
            ...
            if (pictures.Any())
                foreach (var pic in pictures)
                    if (pic.FileType == DbTables.FileType.Profile)
                        return PartialView("DetailPartials/_EmployeeDetailsPartial",
                            new EmployeeDetailsModel
                            {
                                File = new FileModel {FileId = pic.FileId, Type = "employee"},
                                User = user
                            });
            return PartialView("DetailPartials/_EmployeeDetailsPartial",
                new EmployeeDetailsModel {File = new FileModel {Type = "employee"}, User = user});
        }
        ...
        public ActionResult showSponsorDetails(string id, string type, int? sponsorPage)
        {
            ...
            if (pictures.Any())
                foreach (var pic in pictures)
                    if (pic.FileType == DbTables.FileType.Profile)
                        return PartialView("DetailPartials/_SponsorDetailsPartial",
                            new EmployeeDetailsModel
                            {
                                File = new FileModel {FileId = pic.FileId, Type = "employee"},
                                User = user
        ...
        public ActionResult showChildDetails(int id, string type, int? childrenPage)
        {
            ...
            if (pictures.Any())
                foreach (var pic in pictures)
                    if (pic.FileType == DbTables.FileType.Profile)
                        return PartialView("DetailPartials/_ChildDetailsPartial",
                            new ChildDetailsModel
                            {
                                File = new FileModel {FileId = pic.FileId, Type = "child"},
                                Child = child
                            });
            return PartialView("DetailPartials/_ChildDetailsPartial",
                new ChildDetailsModel {File = new FileModel {Type = "child"}, Child = child});
        }
        ...
        public ActionResult ChildEdit(ChildCreateModel model, int? childrenPage)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    ViewBag.Success = "Fadderbarnet " + model.Child.Fname + " " + model.Child.Lname + " ble oppdatert" +
                                      sponsormsg;
        ...
        public async Task<ActionResult> employeeDeactivate(string id, int? employeePage)
        {
            ...
            if (currentUser.RoleNr >= user.RoleNr) //rolenumber is too low, return error message
            {
                ...
                return FilterResultEmployees(search, active, order, filter, employeePhone, EmployeeStreetadress,
                    EmployeeZip, EmployeeCity, EmployeeCounty, EmployeePosition, employeeAccountNumber, pageNumber);
            }
        ...
        public async Task<ActionResult> EditEmployee(changeUserInfo model, int? employeePage)
        {
            ...
            if (ModelState.IsValid)
            {
                var store = new UserStore<ApplicationUser>(_context);
                var manager = new UserManager<ApplicationUser>(store);
                var user = manager.FindById(model.Id); //the user to be edited
                var currentUser = manager.FindByIdAsync(User.Identity.GetUserId()).Result; //the current user
                ...
                if (model.RoleNr != null)
                {
                    ...
                    if ((currentUser.Id == user.Id) && (user.RoleNr != model.RoleNr))
                        //the user wants to change their own role, not allowed
                    {
                        ViewBag.Id = user.Id;
                ...
                if (results.Any())
                    foreach (var appUser in results)
                        if (appUser.Id != model.Id) //if the one using the email is not the user to edit we cannot edit
                        {
                            ...
                            return FilterResultEmployees(search, active, order, filter, employeePhone,
                                EmployeeStreetadress, EmployeeZip, EmployeeCity, EmployeeCounty, EmployeePosition,
                                employeeAccountNumber, pageNumber);
                        }
                ...
                if (result.Succeeded) //if update succeeds
                {
                    if (model.Password != null)
                    {
                        ...
                        var resetToken = await manager.GeneratePasswordResetTokenAsync(model.Id);
                        var Presult = await manager.ResetPasswordAsync(model.Id, resetToken, model.Password);
                        //reset passwor with token
                ...
                if (results.Any())
                    foreach (var appUser in results)
                        if (appUser.Id != model.Id)
                        {
                            ViewBag.Id = user.Id;
                            ViewBag.Error = "Emailen er allerede i bruk";
                            return FilterResultSponsors(search, active, order, filter, SponsorPhone, SponsorStreetadress,
                                SponsorZip, SponsorCity, SponsorCounty, pageNumber);
                        }
                //so far so good, change the details of the user
                user.Fname = model.Fname;
                user.Lname = model.Lname;
        ...
        public async Task<ActionResult> CreateEmployee(RegisterViewModel model, int? employeePage)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var currentUser = userManager.FindByIdAsync(User.Identity.GetUserId()).Result; //the current user
            ...
            var errormsg = "";
            foreach (var modelState in ViewData.ModelState.Values)
                //modelstate errors. get the errors and return to view
                foreach (var error in modelState.Errors)
                    errormsg += error.ErrorMessage + " ";
            ViewBag.Error = "Noe gikk galt: " + errormsg;
            return FilterResultEmployees(search, active, order, filter, employeePhone, EmployeeStreetadress, EmployeeZip,
        ...
        public ActionResult CreateChild(ChildCreateModel model, int? childrenPage)
        {
            ...
            if (ModelState.IsValid) //check if model values are valid
            {
                ...
                try
                {
                    ...
                    ViewBag.Success = "Fadderbarnet " + model.Child.Fname + " " + model.Child.Lname +
                                      " ble lagt til i databasen" + sponsormsg;
                    ViewBag.Id = model.Child.Id;
                    return FilterResultChildren(search, active, order, filter, ChildDoB, ChildSponsor, pageNumber);
                }
                catch (EntityException ex)
            ...
            var errormsg = "";
            foreach (var modelState in ViewData.ModelState.Values) //add modelstate errors, return them to view
                foreach (var error in modelState.Errors)
                    errormsg += error.ErrorMessage + " ";
        ...
        public ActionResult EmployeeDelete(string id, int? employeePage)
        {
            ...
            try
            {
                ...
                var images = (from s in _context.Files
                    //check if the user has profile image. if so delete this too
                ...
                //remove tagboxes of user from database
        ...
        public PartialViewResult FilterResultChildren(string search, string active, string order, string filter,
            string dob, string sponsor, int pageNumber)
        {
            ...
            if ((pageNumber > 1) && children.Any())
            {
                if (children.Count() <= pageSize*pageNumber)
                    //chech that page is not empty. if so show highest available page
                {
                    pageNumber = (int) Math.Ceiling(children.Count()/(double) pageSize);
        ...
        public PartialViewResult FilterResultEmployees(string search, string active, string order, string filter,
            string phone, string streetadress, string zipcode, string city, string county, string position,
            string accountnumber, int pageNumber)
        {
            ...
            if ((pageNumber > 1) && employees.Any())
            {
                ...
                {
                    pageNumber = (int) Math.Ceiling(employees.Count()/(double) pageSize);
                    ViewBag.employeePage = pageNumber;
                    return PartialView("ListPartials/_EmployeePartial",
        ...
        public PartialViewResult FilterResultSponsors(string search, string active, string order, string filter,
            string phone, string streetadress, string zipcode, string city, string county, int pageNumber)
        {
            ...
            if ((pageNumber > 1) && sponsors.Any()) //make sure page is not empty
            {
                ...
                {
                    pageNumber = (int) Math.Ceiling(sponsors.Count()/(double) pageSize);
        ...
        public ActionResult GetUsers()
        {
            ...
            foreach (var user in users)
            {
                ...
                enteties.Add(new {Name = user.Fname + " " + user.Lname, user.Id, imgId, email = user.Email});
            }
