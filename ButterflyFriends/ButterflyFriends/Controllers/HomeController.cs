...
namespace ButterflyFriends.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            if (stripeList.Any())
                if (stripeList.First().Enabeled) ViewBag.StripePublic = stripeList.First().Public;
        ...
        public ActionResult RequestMembership()
        {
            ...
            if (GoogleCaptchaList.Any())
                ...
                if (GoogleCaptchaList.First().Enabeled) model.SiteKey = GoogleCaptchaList.First().SiteKey;
            ...
            {
                Terms = terms.First();
                if (Terms.Enabeled && (Terms.Terms.FileId != 0)) model.TermsID = Terms.Terms.FileId;
            }
            var background = new DbTables.BackgroundImage();
            var backgroundList = _context.BackgroundImage.ToList();
            if (backgroundList.Any())
        ...
        public ActionResult RequestMembership(RequestModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                //check if recaptcha token corresponds to a valid user or not
                ...
                try
                {
                    ...
                    //add request to database, return succsess message
        ...
        {
            var articles =
                _context.Articles.Where(s => s.Published)
                    .OrderByDescending(s => s.FirstPublisheDateTime)
            ...
            else if (startId + 1 > articles.Count)
                //return nothing as startid is as great as there are elements in image list
            {
                articles = null;
            }
            else if (articleNum >= articles.Count)
                //return whole list as there are as many articles as there is articles on one page
            {
                //do nothing
            }
            else
        ...
        public ActionResult GetImages()
        {
            ...
            if (startId < imageNum) return null;
            var manager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext()));
            var currentUser = manager.FindByIdAsync(User.Identity.GetUserId()).Result;
            var images = filterImages(currentUser, startId, imageNum);
        ...
        public ActionResult HandlePayment()
        {
            ...
            if (!string.IsNullOrEmpty(email)) data["receipt_email"] = email;
            else if (user == "true")
